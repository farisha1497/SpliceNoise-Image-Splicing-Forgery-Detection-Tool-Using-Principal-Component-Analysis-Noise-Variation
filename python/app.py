...
def custom_kmeans(data, n_clusters, max_iters=100):
    ...
    for _ in range(max_iters):
        distances = np.linalg.norm(data - centroids.T, axis=2)
...
def PCANoiseLevelEstimator(Ib, patch_size):
    # [PCANoiseLevelEstimator as you wrote it, unchanged]
...
def process_image(input_image_path, output_dir):
    try:
        ...
        valid = np.where(label64 == 0)  # Returns a tuple of arrays (rows, cols)
        if len(valid[0]) == 0:
            raise Exception("No valid blocks found")
        attenfactor = model(meanIb)
        Noise_64c = Noise_64 * attenfactor
        valid_noise = Noise_64c[valid]
        labels = custom_kmeans(valid_noise, 2)
        # Make sure label 2 is the more "anomalous" one
        if valid_noise[labels == 0].mean() < valid_noise[labels == 1].mean():
            labels = 1 - labels  # flip
        result = np.ones(label64.size)
        # Convert valid (tuple of arrays) to flat indices
        valid_indices = np.ravel_multi_index(valid, label64.shape)
        result[valid_indices] = labels + 1  # 1 or 2
